...
class HNSWIndex:
    ...
    def insert(self, node_id: str, vector: List[float]) -> None:
        ...
        if node_id in self.nodes:
            ...
            self.nodes[node_id].vector = vector
            return
        
        # Create new node
        level = self._random_level()
        node = IndexNode(node_id=node_id, vector=vector, level=level)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_connections": total_connections / len(self.nodes) if self.nodes else 0.0,

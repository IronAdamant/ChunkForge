"""
CLI handlers for ChunkForge metadata commands.
...
def cmd_annotate(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    """Add an annotation to a document or chunk."""
    result = chunkforge.annotate(
        target=args.target,
...
def cmd_get_annotations(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    """Retrieve and display annotations."""
    annotations = chunkforge.get_annotations(
        target=args.target,
        target_type=args.type,
        tags=args.tags,
    )
...
def cmd_delete_annotation(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    """Delete an annotation by ID."""
    result = chunkforge.delete_annotation(args.annotation_id)
    if result["deleted"]:
...
def cmd_update_annotation(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    ...
    if args.content is None and args.tags is None:
        ...
        return 1
    result = chunkforge.update_annotation(
...
def cmd_map(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    """Show project map: all documents with chunk counts and annotations."""
    result = chunkforge.get_map()
    if getattr(args, "output_json", False):
...
def cmd_history(args: argparse.Namespace, chunkforge: ChunkForge) -> int:
    """Show change history."""
    entries = chunkforge.get_history(
        limit=args.limit,

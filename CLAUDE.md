...
- **Hybrid search**: HNSW finds 3x candidates, BM25 re-ranks. `alpha * cosine + (1-alpha) * bm25`. Auto-tuned alpha lowers for code-like queries. Both HNSW cosine and BM25 scores are min-max normalized within the candidate set before blending, preventing score clustering in the 0.7-1.0 range. Chunks with Tier 2 agent-supplied signatures get a `TIER2_BOOST = 1.3` multiplier on their vector score before blending, ensuring summary-enriched chunks rank higher for matching queries. `has_agent_signatures(chunk_ids)` on StorageBackend provides the batch lookup.
